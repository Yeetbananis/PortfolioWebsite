...
## Delta (If the stock goes up a dollar, by how much does my option value increase?)
...
2. **Why subtract 1 for puts?**  
   ...
   - Example: A call with delta 84 means an equal put has \( 84 - 100 = -16 \) delta, losing 16 cents of value per the same dollar move up.  

3. **Delta representation.**  
   - Delta is often quoted in whole numbers: e.g., “84 delta” instead of 0.84.  
   - Because each option contract covers 100 shares, \( 0.84 \times 100 = 84 \) delta.  

4. **Stock delta.**  
   - A single share of stock has a delta of 1, since it moves 1:1 with itself.  
   - This fact is crucial in hedging, where traders balance option deltas against stock positions to achieve delta-neutral portfolios.  
...
## Vega (If Implied volatility goes up 1 percent, by how much does my option value increase?)
...
## Theta (Every day that passes, how much value does my option lose)
...
- Examples:
  - Call: \$100 at 5% becomes \$105 in a year. Each passing day reduces time left to earn this extra \$5 → faster decay
...
## Rho (If Interest rates increase by 1 percent, how much value does my option gain?)
