...
> If this was fully true, wouldn’t the market move in smooth lines? How come you see massive jumps in price at seemingly random times?

A massive jump in price suggests an imbalance of information, more specifically there must’ve been an event that triggered a chain reaction of buyers or sellers to bring the price to an area that reflects this new information.
...
You can imagine a stable price movement for Tesla where all investors have accounted for Tesla’s upcoming earnings being potentially disappointing due to a decrease in sales, while also accounting for the likelihood of Tesla being issued government subsidies to increase sales, and all other possible information related to Tesla’s stock price.
...
<div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg my-6 max-w-md shadow-xl">
  ...
  <h3 className="text-white font-bold text-lg leading-tight">
    “NHTSA opens formal investigation into Tesla Autopilot software”
  </h3>
</div>
...
1.  **The Context:** Tesla’s valuation is partly tied to its promise of Full Self-Driving technology.
2.  **The Event:** A federal investigation suggests potential safety flaws or recalls.
3.  **The Risk:** If the software is banned or restricted, the "Tech Company" premium evaporates.
4.  **The Consequence:** This results in lower future margins, delayed features, and a repricing of the stock.

The buyer instantly rushes to change his vote from buy to sell and pushes the price down, with this chain reaction happening on a scale of millions until the price settles somewhere between *“This is wraps for Tesla”* and *“This is actually the best thing that could’ve happened”*.

## Hypothesis and Choosing a Model

My hypothesis is that most of the time all information is priced in, but the market is not truly “efficient”, with the inflow of new information, specifically through news, causing massive jumps in price as investors change their bets.

Now how can this be modeled through numbers that machines understand, more specifically math? Before looking into that I had to settle for what model to use in my project. I had a very wide range of options, ranging from neural network models like LSTM to random forest models (which I won’t cover in depth for brevity purposes), but what I landed on was a **Logistic Regression model**.

I want the model to tell me whether or not a stock will jump or not, but I don’t want a yes or no, I want a more specific probability of that yes or no.



It’s like hiring a student and looking at grades: seeing that they passed all their classes doesn’t give you insight into how well they mastered the material.
...
1.  **Transparency/Explainability of what’s happening:** The model is a “glass box” model because it’s linear (rate of change between variables is constant), so we can always look at the coefficients (weights) of each feature to explain what it learned from data and adjust accordingly or get an answer to why something may not be working out. Other models like neural networks are considered “black box” models because they model non linear relationships, which results in it being hard to decipher why a model made a certain prediction.
2.  **Holds well against data noise and overfitting:** Since it’s a linear model it captures more broad and consistent trends, making it less susceptible to memorizing small details in stock data that aren’t important to the bigger purpose. Neural networks and random forests are capable of seeing micro patterns, which may lead them to learn those patterns too well on training and perform poor on new data (where it matters).
3.  **Speed:** Since it’s a linear model it’s very quick and efficient which makes training and testing a lot swifter. Neural networks and random forests are non linear models so training and testing can take a lot of computational power and time.

Logistic Regression outputs a probability, but how do I judge this probability as a metric and compare different versions of models? I settled on using AUC (Area Under the Curve) as the metric: The model outputs a probability like 0.61 (61%) or 0.85 (85%) chance of a jump happening, and to actually use this you have to pick a threshold “if Prob > 0.85 predict jump”, the problem is how do you know which threshold (0.4, 0.5, 0.6 etc) the model performs best on?
...
The curve is a function that models the trade off between catching real jumps and mislabeling false jumps for real jumps, with the metric AUC simply being the area under that curve (summing up the model’s win rate (TPR) across every single possible threshold (FPR)):

$$
AUC = \int_{0}^{1} TPR(FPR) \, d(FPR)
$$

You start at a strict threshold on the left (scenario 1 above), and as you increase it to a loose one (scenario 2) a good model should initially shoot up very quick (if the model is 95 percent sure of a prediction there should be a very high ratio of true positives to false positives/steep slope), and then die down (if a model is 40 percent sure of a prediction and it’s an accurate model then it’ll get a lot more wrong than right/flat slope):

<ImageWithFullscreen src="/images/AUCGood.png" alt="AUC Good" />

**How does a good model look?** The dotted straight white line above showing a constant slope tells us for every true positive (correct prediction, y axis) it makes a wrong prediction (false positive, x axis) which is just random guessing. The “ROC Curve” neon green line shows the curve of a good model, where for a large amount of correct predictions (true positives, y axis), the slope is very high at first meaning the number of wrong predictions (false positives, x axis) increases by a small amount. This is exactly what we want, a model getting a lot of correct predictions while making as little mistakes as possible. A bad model with low AUC would look like this:

<ImageWithFullscreen src="/images/AUCBad.png" alt="AUC Bad" />

Realistically I aim to get an AUC above 0.75, since that’s the range where I can tune into a strategy to actually profit off of the predictions.

Now that I’ve settled on Logistic Regression and AUC as a way to assess it, I will focus on **Tesla (TSLA)** as my asset of choice. I chose Tesla because it's a volatile stock that often appears in the news, making it a good candidate with a lot of data to analyze. Now how should I approach modelling news as numbers/features the model can actually read (using math)?
...
Which is the base rate of jumps that seem to happen “randomly”. It’s the jumps that happen on quiet days with no news/events, and for a stock like Tesla this could be high (the market is jittery with jumps happening often):
...
**Jump Echo** is a rolling count of recent jumps, it sums up all the recent jumps to give the model a good idea of how choppy the current market is. I defined a jump as a “4 standard deviation event”: The price of Tesla moved an average of 10 cents per minute over the last 60 minutes, a jump would be defined as movement of at least 40 cents in a minute, with all of these being summed up over the last 60 minutes. Implemented in code this is:
...
In theory a 4 standard deviation event should happen once in every 15,873 minutes (0.0063% of the time), however in financial markets this happens significantly more often (again supporting the fact that the efficient markets hypothesis isn’t fully accurate).

I then came up with another feature called **“garch_forecast”**: GARCH stands for “Generalized Autoregressive Condition Heteroskedasticity”, and simply translates to “Volatility Clustering”. As mentioned before it is based on the idea that volatility has memory/clusters, with previous large jumps influencing current jumps, and the output of the formula:
...
The previous feature “jump_echo” sees 0 jumps, however the “garch_forecast” feature sees an increase in the $\varepsilon_{t-1}^2$ term accumulating pressure, and raises the GARCH feature’s value indicating “there’s a jump that may come soon and the pressure is building”, which is useful for the bigger picture model to view so it can pick up on what conditions look like before jumps happen.
...
So I conducted a “misses analysis”, where I extracted a wide range of features and had another model extract the blind spots:
...
💡 INSIGHT: Missed jumps consistently happen during large volume spikes.

--- ANALYSIS OF FALSE POSITIVES (FALSE ALARMS) ---
- Average RSI during a False Alarm: 51.35
- Average RSI during normal times:  49.82
💡 INSIGHT: False alarms often happen when RSI is neutral, indicating high volatility but no clear momentum.
```

This led to 2 new features related to price/volume, the first of which being **Volume spike ratio**. It’s simply a metric of how the current volume (total stock shares traded) compares to the past hour, with higher volume indicating a larger chance of jumps happening. Code implementation:
...
The second feature was **RSI (relative strength index)**, which measures momentum of a stock to indicate whether its relatively “overbought” or “oversold”. The model made mistakes when RSI was neutral (no direction pressure) but there was movement, so feeding it RSI gives it a chance to see that jumps don’t happen as often when RSI is neutral, and may occur more when RSI is high or low. Implemented in code:
...
For a quick example of how RSI works: It’s on a scale of 0 to 100. Above **70** is considered overbought, and below **30** is considered oversold.
...
This feature simply determines if the news is good news or bad news for a certain asset. BERT, which stands for “Bidirectional Encoder Representations from Transformers”, is a natural language processing model trained on tons of data to understand language better, and FinBERT is just a version trained on tons of financial data such as news, analyst reports, earnings calls, etc. to understand financial language in a deep way.
...
<div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg my-6 max-w-md shadow-xl">
  ...
  <h3 className="text-white font-bold text-lg leading-tight">
    “Tesla shares drop as margins tighten”
  </h3>
</div>

The model breaks the sentence down into tokens recognizing “drop” and “margins tighten” in financial context. It maps these onto a vector field of dimension 768 (to capture all the nuances of a headline), and sees “margins tighten” is mathematically close to “negative financial outlook”, and gives a probability to either positive, neutral, or in this case negative.

It outputs a number like **-0.90** for 90 percent negative, and that’s the feature I use so my model can see directional bias and assume the jump will likely be downward.
You may ask “Why does the model need direction bias if it only predicts whether jump will happen and not the direction?”, and this is because in finance stocks tend to go up slowly and crash very quickly.
...
<div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg my-6 max-w-md shadow-xl">
  ...
  <h3 className="text-white font-bold text-lg leading-tight">
    “Tesla unveils new Optimus robot”
  </h3>
</div>
...
It finds the answers to “Is a robot more similar to ‘Earnings’ or ‘Product’”, by assigning probabilities to each category using a similar method to the BERT model, where it maps the article onto a vector field and checks mathematical similarity:

* **Match with “Earnings”:** 2% (physically far from earnings in the vectorspace)
* **Match with “Regulatory”:** 5%
* **Match with “Product”:** 93% (physically close to earnings in the vectorspace)

It then outputs a distinct answer of the category based on the highest probability, ‘Product’ in this case, and I use this category as a feature.
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  <div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg">
    ...
    <h3 className="text-white font-bold text-md mt-1">“Tesla halts lines due to fire.”</h3>
  </div>
  <div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg">
    <span className="text-xs font-mono text-neutral-500 uppercase">Headline B</span>
    <h3 className="text-white font-bold text-md mt-1">“Factory stops after blaze”</h3>
  </div>
</div>

It sees “halts” and “stops” as two different words even though the two articles represent the same idea as far as a jump predicting model should be concerned.
...
In this case `pca_0` may represent “Disaster Intensity” with both headlines scoring 8.5, and `pca_1` may represent “Company problems” with both headlines scoring 5.0.
The model can learn that high `pca_0` and mildly high `pca_1` results in a jump and capture context the previously engineered features didn’t represent.
...
Stocks jump when there’s news that’s not commonly seen or surprising.
I used an SVM model, standing for “Support Vector Machine” which is a machine learning algorithm used for classification.
It looks at thousands of past Tesla headlines that I fed it, and creates a cluster in a mathematical vector space.
This cluster contains commonly seen news articles for Tesla like “Price prediction raised”, “New model 3”, etc (boring for this stock), and when a new headline arrives the SVM measures the distance from the center of that blob and outputs a score from -5.0 (very boring news) to +5.0 (very unique never seen news).
...
<div className="bg-neutral-900 border border-neutral-700 p-4 rounded-lg my-6 max-w-md shadow-xl">
  ...
  <h3 className="text-white font-bold text-lg leading-tight">
    “Elon Musk tweets he is buying Twitter”
  </h3>
</div>

is fed, the SVM has never seen the words buying and Twitter in the same headline before and the point is plotted very far from the center of the cluster.
The model then outputs a score of **+4.5** signaling “Extreme rare” headline, which the Logistic Regression model can learn to read as a potential signal for incoming volatility.

I ran the feature engineering script to train the models and output 2 years worth of engineered features:
```text
--- Step 3: Engineering All Features on Combined Data ---
  > [1/4] Engineering Price & Volume Features...
  > [2/4] Engineering News Features...
Device set to use cuda:0
  -> FinBERT Sentiment: 100%|███████████████████████████████████████████████████████████████████████████████████████████████| 1049/1049 [00:00<?, ?it/s]
Device set to use cuda:0
  -> Topic Modeling: 100%|██████████████████████████████████████████████████████████████████████████████████████| 1049/1049 [00:00<00:00, 520073.86it/s]
Batches: 100%|██████████████████████████████████████████████████████████████████████████████████████████████████████████| 33/33 [00:00<00:00, 38.35it/s]
    -> Fitting and Saving PCA...
    -> Fitting and Saving SVM (High-Precision Mode)...
  > [3/4] Fusing Price and News features...
  > [4/4] Final cleaning and saving...
```

After training was complete I ran a confirmation sanity test on the SVM and PCA models (the ones that weren't pre-trained):
```text
STARTING MODEL CAPABILITIES DEMO
========================================

  > Loading Brains...
  > System Ready.

DEMO 1: PCA Contextual Mapping
   (Comparing semantic meaning while controlling for the word 'Tesla')

  Headline A: 'Tesla halts Berlin production due to fire'
  Headline B: 'Tesla factory lines stopped after massive blaze'
  Headline C: 'Tesla releases Q3 earnings report'

  Mathematical Distance:
     A <-> B (Synonyms):  0.2289
     A <-> C (Different): 0.3956
  SUCCESS: Synonyms are closer than different topics.


DEMO 2: SVM Anomaly Detection
     SCORE | STATUS     | HEADLINE
  ------------------------------------------------------------
  -0.8608 | [NORMAL]   | 'Tesla stock rises 0.5% in pre-market trading'
  -0.5249 | [NORMAL]   | 'Analyst maintains Hold rating on Tesla stock'
  -0.3008 | [NORMAL]   | 'S&P 500 closes slightly higher on Tuesday'
  +0.5374 | [ALERT]   | 'Elon Musk challenges Zuckerberg to cage fight'
  +0.3289 | [ALERT]   | 'Tesla factory destroyed by alien laser beam'
  +0.4457 | [ALERT]   | 'Tesla CEO arrested for bank robbery'
========================================

(quant_news_model) PS C:\Users\timof\Desktop\quant_news_project> 
```
The PCA model correctly determined headline A was close to B (although not extremely precise), and the SVM model correctly classified all of the articles as either anomalies or boring.
...
**Step C: The Gradient Calculation** The Referee calculates the error between the prediction and reality (data) ($1.0 - 0.5 = 0.5$). Simply knowing how much the model was off isn’t enough, it also needs to know what way to be nudged to improve. This is where we calculate the Gradient Vector ($\nabla J$). The Gradient takes the partial derivative of each variable (Error, RSI, Volume etc), meaning you get the direction of the steepest uphill climb in this landscape.
...
A gradient of zero means the model reached a point where there is no further improvement, so the weights are locked or “learned”. The entire process is visualized below:
...
### 4. The Verdict  
The model outputs a 61% probability of a jump happening right now. I personally don’t want to watch the model consume minute by minute data until it outputs a good probability so i could place some complex multi-legged options trade to profit off volatility. Since this model predicts volatility jumps and volatility itself isn’t a “buy the stock now” signal, it would be useful to develop a strategy that utilizes this prediction with some rules to place a trade that actually profits off of this.

# Putting the Model to Work (Algorithmic Trading Setup)

To utilize the model’s volatility prediction, I created a rule based automated trading strategy and then backtested this strategy with realistic market factors to filter out blind optimism and mimic a real trading environment.
...
**How are the specific values determined?** At first I attempted running through various value combinations to train on the entire data set of 2 years, however I quickly noticed the portfolio would crash since during that timeline the market changed behavior multiple times and what worked for 6 months stopped working later on. That’s where I decided to come up with an adaptive regime system:
...
* **Cooldown (10-60 mins):** The model sometimes exited trades after riding a trend and then very shortly re-entered on the same trend, except price was near the top of the trend and it would reverse and hit the stoploss. To combat this, I found that a range of 10-60 minutes of cooldown between each trade would give the model time to “reset” and re-enter when an unrelated new setup was present.

**Risk Management & Logic**

For risk to reward, if the model is wrong in the direction of the jump you want to exit swiftly since jumps in stock price can turn into large price trends with large downside, so the stoploss should be kept tight. To specifically calculate the value I found that taking ATR (current volatility) and multiplying it by a number in the **1.25–1.75** range cuts losses fast while not being exited from the trade by noise.
...
On the flip side, if you are correct about the direction of the jump, the jumps usually compound into a trend and you generate the most profit by riding the trend, so take profit ATR values (which work similar to the stoploss ones) are in a higher **5–10** range.
...
* **ATR Filter:** This is a bit more nuanced and is designed to combat profit from small moves being killed by fees. ATR (Average True Range) is simply a volatility metric calculated on minute data, practically speaking it’s how much the stock is moving in dollars per minute on average.
...
You want the market to be volatile enough (ATR to be above a certain threshold) to combat “useless” or small loss trades due to fees since stoploss and take profit values are measured using ATR, so the ATR filter range of at least 20–40 cents combats this.
...
A Sharpe ratio answers the question “How stable is the profit I'm making” or in more formal terms what is the risk adjusted return of my strategy:
...
From the formula above you can see the risk it measures is in respect to volatility, so higher volatility means higher risk and in result a lower Sharpe ratio, which is useful to know but there’s one caveat. Volatility is directionless so a 50 percent volatility could mean 50 percent gain or 50 percent loss, but for a strategy made for maximizing profit we don’t want to penalize upside volatility (since it's good to make money fast), only downside volatility, so we should use a similar metric that doesn’t penalize upside volatility and that metric is called the Sortino ratio:
...
--- Step 1: Train 2024-01-02 to 2024-07-02 ---
  ...
  Optimizing: 100%|█████████████████████████████████████████████████| 65/65 [00:13<00:00,  4.82it/s]
  > Best Sortino: 6.23 (Sharpe: 2.89)
...
--- Step 2: Train 2024-02-02 to 2024-08-02 ---
  ...
  Optimizing: 100%|█████████████████████████████████████████████████| 65/65 [00:03<00:00, 18.17it/s]
...
--- Step 3: Train 2024-03-02 to 2024-09-02 ---
  ...
  Optimizing: 100%|█████████████████████████████████████████████████| 65/65 [00:03<00:00, 17.87it/s]
...
--- Step 4: Train 2024-04-02 to 2024-10-02 ---
  ...
  Optimizing: 100%|█████████████████████████████████████████████████| 65/65 [00:03<00:00, 18.13it/s]
...
--- Step 15: Train 2025-03-02 to 2025-09-02 ---
  ...
  Optimizing: 100%|█████████████████████████████████████████████████| 65/65 [00:03<00:00, 17.54it/s]
...
--- 🏆 OVERALL WALK-FORWARD OOS RESULTS 🏆 ---
...
<div className="border border-neutral-800 bg-neutral-900/50 rounded-xl p-6 my-8 space-y-6">
  ...
  <div className="text-sm text-neutral-300 space-y-4">
    ...
    <div>
      The bar range gives how fast the market is moving (2$ a bar vs 5 cents), the slippage factor is how much of the bar’s total move you expect to lose to the quick move, and trade size over volume gives increased penalty if you make up a higher percentage of the bar’s volume.
    </div>
    ...
    <div className="bg-neutral-800/30 border border-neutral-700/50 rounded-lg p-5 my-4 text-xs">
      ...
      <div className="font-mono text-neutral-200 ml-4 space-y-1">
        <div>Impact = 0.10 ^ 0.7 ≈ 0.1995</div>
        <div>Penalty = 0.08 × 0.1995 × $1.00</div>
      </div>
...
Finally brokers take fees for trading, usually not a large amount but if this isn’t modeled then you can buy 1000 shares, sell for \$0.01 profit and make \$10, but in reality you pay \$5 to enter and \$5 to exit resulting in a profit of \$0. I model this with commissions: 0.005$ per share, applied to entry and exit.
